import 'package:flutter/material.dart';

class ColorsManager {
  static const Color darkBackground = Color(0xff03031D);
  static const Color darkContainer = Color.fromARGB(255, 7, 7, 44);
  static const Color darkPrimary = Color(0xff30B0C7);
  static const Color lightBackground = Color(0xfff5f5f5);
  static const Color lightContainer = Color(0xffe0e0e0);
  static const Color lightPrimary = Color(0xff30B0C7);
  static const Color red = Color.fromARGB(255, 199, 48, 63);
  static const Color blue = Color.fromARGB(255, 48, 176, 199);
  static const Color darkTr = Color(0xff227697);
  static const Color darkGrey = Color(0xff49454F);
  static const Color lightgrey = Color.fromARGB(255, 99, 94, 107);
}
